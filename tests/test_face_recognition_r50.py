    ...
    try:
        ...
        if result.face_landmarks:
            ...
            if face_landmarks:
                ...
                # 计算新的边界框，增加边距
                new_width = face_width * 1.5  # 增加 50% 的宽度
                new_height = face_height * 1.5  # 增加 50% 的高度
                
                # 计算新的边界框坐标
                x1 = max(0, int(center_x - new_width // 2))
                y1 = max(0, int(center_y - new_height // 2))
                x2 = min(image_resized.shape[1], int(center_x + new_width // 2))
                y2 = min(image_resized.shape[0], int(center_y + new_height // 2))
...
def test_with_camera():
    ...
    face_recognition_model = FaceRecognitionModel(
        str(model_dir / 'weights' / 'w600k_r50.onnx')
    )
    ...
    while cap.isOpened():
        ...
        # 显示结果
        cv2.imshow('Face Recognition (w600k_r50)', frame)
...
def test_with_image(image_path):
    ...
    # 显示结果
    cv2.imshow('Face Recognition - Image Test (w600k_r50)', image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

if __name__ == '__main__':
    print("人脸识别模型测试 (w600k_r50)")
    print("=" * 50)
